...
function convertToWords(number) {
    if (number === 0) {
        ...
        if (number === 100) {
            ...
            return (wordMap[hundred] + " hundred " + wordMap[tens] + "-" + wordMap[ones]);
        }
        ...
        if (number === 1000) {
            ...
            if (ones === 0) {
                ...
                return wordMap[thousand] + " thousand " + wordMap[tens] + '-' + wordMap[ones];
            }
        } else if (hundred !== 0) {
            if (ones === 0 && tens === 0) {
                return wordMap[thousand] + " thousand " + wordMap[hundred] + ' hundred ';
            }else if (ones === 0) {
                return wordMap[thousand] + " thousand " + wordMap[hundred] + ' hundred ' + wordMap[tens];
            } else if (tens === 0){
                return wordMap[thousand] + " thousand " + wordMap[hundred] + ' hundred ' + wordMap[ones];
            } else{
                return wordMap[thousand] + " thousand " + wordMap[hundred] + ' hundred ' + wordMap[tens] + '-' + wordMap[ones];
            }
        }
    }
}
